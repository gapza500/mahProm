...
func riderCuteCard<Content: View>(_ title: String, gradient: [Color], @ViewBuilder content: () -> Content) -> some View {
    ...
    .background(
        ZStack {
            RoundedRectangle(cornerRadius: DesignSystem.Metrics.cardCornerRadius).fill(DesignSystem.Colors.cardSurface)
            RoundedRectangle(cornerRadius: DesignSystem.Metrics.cardCornerRadius).fill(
