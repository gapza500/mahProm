...
struct OwnerConsultConversationView: View {
    @StateObject private var viewModel: ConsultConversationViewModel

    init(session: ConsultationSession, profile: UserProfile) {
        _viewModel = StateObject(wrappedValue: ConsultConversationViewModel(session: session, senderProfile: profile, senderRole: .owner))
    }
    ...
    var body: some View {
        ...
        .navigationTitle(viewModel.session.vetName ?? "Consultation")
    ...
    private func bubble(for message: ChatMessage) -> some View {
        ...
        VStack(alignment: isMe ? .trailing : .leading, spacing: 4) {
            Text(message.text)
                .padding(10)
                .background(isMe ? Color(hex: "FF9ECD") : Color.white)
