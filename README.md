...
- **PetReady Owner**: TabView with Home → Pets → Health → Clinics → Chat → Info → Settings. Health + Clinics tabs now pull real records/appointments and the Chat tab connects to the smart vet queue stub so owners can start consultations and exchange messages.
- **PetReady VetPro**: Dashboard/Patients/Queue/Content/Settings tabs with cute cards, patient placeholders, and links into pending Clinic features. Queue + Clinic Appointments tabs now reflect the shared communication/booking services so vets can approve bookings and monitor waiting owners.
- Owner “Health” tab now surfaces real vaccine/treatment data via `HealthRecordService` and provides a shareable Health QR pass. The Clinics tab pulls live partner data, map pins, and allows in-app appointment booking that syncs with VetPro.
...
- **PetReady VetPro**: Dashboard gains a “Clinic Appointments” action for approving owner bookings; future bookings land under the new ClinicAppointmentsView.
